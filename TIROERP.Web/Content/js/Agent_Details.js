...
//update agent personal details
$(document).on("click", "#UpdateAgentDetails", function () {
    $('#progressBar').show();
    //fetch agent details
    ...
    var formData = JSON.stringify({
        ...
        "CONDITIONAL_OPERATOR": "UPDATE_AGENT_PERSONAL_DETAILS"
    });
...
function callAjaxFunctionAndLoadData(formData) {
    ...
    $.ajax({
        ...
        traditional: true,
        success: function (d) {
            if (d.CONDITIONAL_OPERATOR === "UPDATE_AGENT_PERSONAL_DETAILS") {
                ...
                $(".locationname").text(d.CURRENT_LOCATION_NAME);
                $("#CURRENT_LOCATION").val(d.CURRENT_LOCATION).trigger("chosen:updated");

                //$("#divUSER_EMAIL").html(d.USER_EMAIL);

                if (d.FILE_PATH !== null && d.FILE_PATH.length > 0) {
                    var fullFilePath = '/Uploads/AgentUploadedFiles/' + d.FILE_PATH;
                ...
                _.each(d.LST_USER_ADDRESS, function (value) {
                    row += "<tr id ='tr_add_" + value.ADDRESS_TYPE_ID + "'><td><span class='user-address'>" + value.ADDRESS_TYPE + "-" + value.ADDRESS + "</span></td><td>" + value.USER_COUNTRY + "</td><td>" + value.USER_STATE + "-" + value.CITY_CODE + "</td><td>" + value.USER_VILLAGE + "</td><td>" + value.USER_PINCODE + "</td> "
